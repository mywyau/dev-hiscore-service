...
class LevelServiceImpl[F[_] : Concurrent : Logger](
  ...
  override def awardLanguageXpWithLevel(
    ...
    for {
      maybeSkill <- languageConnector.getLanguage(devId, language)
      currentXp = maybeSkill.map(_.xp).getOrElse(BigDecimal(0))
      newTotalXp = currentXp + xpToAdd
      newLevel = calculateLevel(newTotalXp)
      nextLevel = newLevel + 1
      nextLevelXp = levelThresholds.lift(newLevel).getOrElse(levelThresholds.last)
      result <- languageConnector.awardLanguageXP(devId, username, language, newTotalXp, newLevel, nextLevel, nextLevelXp)
    } yield result
}
...
object LevelService {
  ...
  def apply[F[_] : Async : Logger](
    skillsConnector: SkillsConnectorAlgebra[F],
    languageConnector: LanguageConnectorAlgebra[F]
  ): LevelServiceAlgebra[F] = new LevelServiceImpl[F](skillsConnector, languageConnector)
}
