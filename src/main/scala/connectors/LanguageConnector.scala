...
trait LanguageConnectorAlgebra[F[_]] {
  ...
  def getLanguage(userId: String): F[Option[LanguageData]]

  def getAllLanguageData(userId: String): F[List[LanguageData]]
}

final class LanguageConnector[F[_] : Async : Logger](client: Client[F], baseUri: Uri) {

  def getLanguageData(userId: String): F[Option[LanguageData]] = {
